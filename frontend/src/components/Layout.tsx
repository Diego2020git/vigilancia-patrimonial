...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app"><aside>{links.map(l => <Link key={l} to={`/${l.toLowerCase().replaceAll(' ','-')}`}>{l}</Link>)}<button onClick={()=>setAuth(null,null)}>Sair</button></aside><main>{children}</main></div>
}
